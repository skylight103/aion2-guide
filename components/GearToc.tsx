...
export function GearToc() {
  const [active, setActive] = useState<GearChapterId>(gearChapters[0].id);
  const [stuck, setStuck] = useState(false);
  const sentinelRef = useRef<HTMLDivElement>(null);
  const jumping = useRef<GearChapterId | null>(null);
  const jumpGen = useRef(0);

  useEffect(() => {
    const nodes = [...document.querySelectorAll<HTMLElement>("[data-gear-chapter]")];
    ...
    const sync = () => {
      ...
      let current: GearChapterId = gearChapters[0].id;
      for (const node of nodes) {
        if (node.getBoundingClientRect().top <= line) {
          current = node.id as GearChapterId;
        }
      }
      setActive(current);
    };
  ...
  const jump = (id: GearChapterId) => (event: MouseEvent<HTMLAnchorElement>) => {
